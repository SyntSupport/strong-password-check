        ...
        def before_save_with_pass_check
            if self.password_confirmation
              if self.password.match(/^#{self.login}/)
                self.errors.add_to_base(l(:message_your_pass, :pass => self.password, :points => 0, :log => l(:start_with_name)))
                return false
              end
              isstrongenough, points, strlog = strong_enough_pass(self.password)
              if !isstrongenough
                self.errors.add_to_base(l(:message_your_pass, :pass => self.password, :points => points, :log => strlog)[0..-3])
                return false
              end
            end
